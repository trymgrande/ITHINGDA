...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        """
        if all([len(i) == 1 for i in assignment.values()]):
            return assignment
        ...
        for val in assignment[var]:
            ...
            if not self.inference(curr_assignment, queue):
                continue
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for v in self.variables:
            if len(assignment[v]) > 1:
                l.append((assignment[v], v))   
        el = min(l, key=lambda x: len(x[0]))[1]
        return el
    ...
    def revise(self, assignment, i, j):
        ...
        """
        removed = False
        if i not in assignment.keys() or j not in assignment.keys():
            return False
