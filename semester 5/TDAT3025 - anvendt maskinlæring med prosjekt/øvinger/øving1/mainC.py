...
with open('day_head_circumference.csv') as csv_file:  # read dataset from file
...
class LinearRegressionModel:
    ...
    def f(self, x):
        ...
        return 20 * s((x @ self.W) + self.b) + 31
        # return x*1.1
...
plt.xlabel('x')
plt.ylabel('y')
x = torch.arange(torch.min(x_train), torch.max(x_train), 1.0, device="cuda").reshape(-1, 1)
y = model.f(x).detach().cpu()
plt.plot(x.cpu(), y, label='$y = f(x) = 20 \\sigma(Wx + b) + 31$ \n $  \\sigma(z) = \\frac{1}{1 + e^{-z}} $')
plt.legend()
plt.show()