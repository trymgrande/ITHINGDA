...
SELECT leilighet.ant_rom, COUNT(ant_rom) AS "Antall leiligheter" FROM leilighet LEFT JOIN bygning ON leilighet.bygn_id=bygning.bygn_id WHERE ant_rom = 5 GROUP BY ant_rom
...
SELECT bygning.postnr, COUNT(andelseier.and_eier_nr) FROM bygning INNER JOIN leilighet ON(bygning.bygn_id = leilighet.bygn_id) INNER JOIN andelseier ON(leilighet.and_eier_nr = andelseier.and_eier_nr) GROUP BY bygning.postnr;